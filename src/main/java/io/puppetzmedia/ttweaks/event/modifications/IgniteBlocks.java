...
public class IgniteBlocks {
    ...
    public static void init() {
        ...
        if (ModList.get().isLoaded("chisel")) {
            ...
            for (String plank : blocks) {
                Optional<Block> chiselPlank = TTUtilities.getBlock("chisel", plank);
                chiselPlank.ifPresent(IgniteBlocks::setFireValues);
            }
        }
        ...
        if (ModList.get().isLoaded("tconstruct")) {
            ...
            Optional<Block> tinkerToolTable = TTUtilities.getBlock("tconstruct", "tooltables");
            tinkerToolTable.ifPresent(IgniteBlocks::setFireValues);
        }

        // If the user has defined some entries. Try to add properties too them.
        //for (
       //         String customEntry : TTConfiguration.tweaks.igniteBlocks.customBlocks) {
    ...
    private static void getBlockForEntry(String entry) {
        ...
        TwistedTweaks.LOGGER.log(Level.INFO, String.format("[TT] Custom block was found! Adding fire properties to %s.", block.get().getRegistryName()));
        setFireValues(block.get(), encouragement, flammability);
    }
